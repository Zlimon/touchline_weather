...
try:
    ...
    _LOGGER.error("pytouchline_extended not installed. Install with: pip install pytouchline-extended")
    raise
...
DEFAULT_BASE_TEMP = 20.0
DEFAULT_ADJUSTMENT_FACTOR = 0.5
...
PRESET_MODES = {
    ...
    "Auto Weather": PresetMode(mode=0, program=0),
}
...
PLATFORM_SCHEMA = CLIMATE_PLATFORM_SCHEMA.extend({
    ...
    vol.Optional(CONF_WEATHER_ADAPTATION, default=True): cv.boolean,
    ...
    vol.Optional(CONF_SONOFF_SENSORS, default={}): {cv.positive_int: cv.string},
})
    ...
    async def _async_update_data(self):
        ...
        try:
            ...
            if self.device_count is None:
                try:
                    device_count_raw = await self.hass.async_add_executor_job(
                        self.py_touchline.get_number_of_devices
                    )
                    self.device_count = int(device_count_raw)
                    _LOGGER.debug(f"Found {self.device_count} Touchline devices")
                except Exception as err:
                    _LOGGER.error(f"Failed to get device count: {err}")
                    raise
                ...
                for device_id in range(self.device_count):
                    try:
                        ...
                        _LOGGER.error(f"Failed to initialize device {device_id}: {err}")
                        raise
            ...
            for device_id, device in self.devices.items():
                try:
                    ...
                    _LOGGER.warning(f"Failed to update device {device_id}: {err}")
...
class WeatherManager:
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(url, headers=headers) as response:
                    ...
                    # Get temperature exactly 3 hours from now
                    forecast_items = data.get('properties', {}).get('timeseries', [])
                    if not forecast_items:
                        _LOGGER.warning("No forecast data available from Yr API")
                        return
                    ...
                    for item in forecast_items:
                        ...
                        if forecast_time_str:
                            try:
                                ...
                                _LOGGER.warning(f"Error parsing forecast time {forecast_time_str}: {err}")
                                continue

                    if closest_forecast:
                        temp_data = closest_forecast.get('data', {}).get('instant', {}).get('details', {})
                        self.forecast_temp_3h = temp_data.get('air_temperature')

                        if self.forecast_temp_3h is not None:
                            _LOGGER.info(f"Temperature forecast for 3 hours from now: {self.forecast_temp_3h}°C")
                            await self.notify_listeners()
                        else:
                            _LOGGER.warning("No temperature data in forecast")
                    else:
                        _LOGGER.warning("Could not find suitable 3-hour forecast data")

        except asyncio.TimeoutError:
            _LOGGER.error("Timeout when fetching weather data from Yr API")
        except Exception as err:
    ...
    async def notify_listeners(self):
        ...
        for callback in self._callback_listeners:
            try:
                ...
                _LOGGER.error(f"Error in weather update callback: {err}")
    ...
    def calculate_target_temperature(self, base_comfort_temp, current_room_temp=None):
        ...
        night_mode_start = datetime.time(0, 0)
        night_mode_end = datetime.time(4, 0)
...
async def async_setup_platform(
    ...
    try:
        ...
        sonoff_sensors = config.get(CONF_SONOFF_SENSORS, {})
        ...
        if not host:
            _LOGGER.error("Host configuration is required")
            raise PlatformNotReady("Host configuration is required")

        # Create base PyTouchline instance
        try:
            py_touchline = PyTouchline(url=host)
        except Exception as err:
            _LOGGER.error(f"Failed to create PyTouchline instance: {err}")
            raise PlatformNotReady(f"Failed to create PyTouchline instance: {err}")
        ...
        try:
            ...
            _LOGGER.error(f"Failed to fetch initial data from Touchline: {err}")
            raise PlatformNotReady(f"Failed to fetch initial data from Touchline: {err}")

        # Setup weather manager
        weather_manager = None
        if weather_adaptation and latitude is not None and longitude is not None:
            ...
            try:
                ...
                _LOGGER.warning(f"Initial weather update failed: {err}")
        ...
        for device_id in range(coordinator.device_count):
            ...
            sonoff_sensor = sonoff_sensors.get(device_id_plus_one)
            default_adaptive = device_id_plus_one in default_adaptive_devices

            device = SonoffIntegratedTouchline(
                coordinator,
                device_id,
                weather_manager,
                weather_adaptation,
        ...
        _LOGGER.info(f"Successfully set up {len(devices)} Touchline devices")

    except Exception as err:
        _LOGGER.error(f"Failed to set up Touchline platform: {err}")
        raise


class SonoffIntegratedTouchline(ClimateEntity):
    ...
    async def update_automatic_temperature(self):
        """Update target temperature automatically based on weather and Sonoff sensor."""
        ...
        try:
            ...
            if self._sonoff_sensor:
                try:
                    ...
                    _LOGGER.warning(f"Invalid temperature reading from Sonoff sensor {self._sonoff_sensor}: {err}")
            ...
            if current_target is None:
                _LOGGER.warning(f"Could not get current target temperature for {self.name}")
                return
            ...
            _LOGGER.error(f"Error in automatic temperature update for {self.name}: {err}")
    ...
    async def async_set_preset_mode(self, preset_mode):
        ...
        try:
            ...
            if not self._auto_mode:
                ...
                mode_settings = PRESET_MODES.get(preset_mode)
            ...
            _LOGGER.error(f"Error setting preset mode for {self.name}: {err}")
    ...
    async def async_set_temperature(self, **kwargs: Any) -> None:
        ...
        try:
            ...
            _LOGGER.error(f"Error setting temperature for {self.name}: {err}")
    ...
    def current_temperature(self):
        ...
        try:
            ...
            _LOGGER.warning(f"Error getting current temperature for {self.name}: {err}")
    ...
    def target_temperature(self):
        ...
        try:
            ...
            _LOGGER.warning(f"Error getting target temperature for {self.name}: {err}")
    ...
    def preset_mode(self):
        ...
        try:
            ...
            _LOGGER.warning(f"Error getting preset mode for {self.name}: {err}")
